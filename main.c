...
#define WALL_AA1 1                        // Anti-Aliasing for frameame ON 
...
int main()
{
    ...
    // setting vertex and color of wire frameame
    SetWallVertex(wallF);
    SETVECTOR(gray,0x38, 0x38, 0x38, 0x80);

    {
      const u_long giftag0[2] = { SCE_GIF_SET_TAG(0, 1, 0, 0, 0, 1), 0x000000000000000eL };
    ...
    {
        int oddeven;
        const u_long giftag_cube[2] = { SCE_GIF_SET_TAG(0, 1, 0, 0, 1, 9), 0x0000000515151510L };
        const u_long giftag_wall[2] = { SCE_GIF_SET_TAG(0, 1, 0, 0, 1,  4), 0x0000000000005510L };
        u_int j;
        ...
        // make matrix transforming frameom Local to World
        sceVu0TransMatrix(WorldLocalMatrix, RotMatrix, trans);
        // make lighting matrix
        sceVu0MulMatrix(LocalLightMatrix, NormalLightMatrix, WorldLocalMatrix);
        
        // make matrix transforming frameom World to View
        create_viewworld(ViewWorldMatrix, camera_rot, camera_p, camera_zd, camera_yd);
        
        // make matrix to transform frameom View to Screen
        sceVu0ViewScreenMatrix(ScreenViewMatrix, 512.0f, 1.0f, 0.47f,
                            2048.0f, 2048.0f, 1.0f, 16777215.0f,
                            64.0f,65536.0f);

        sceVu0MulMatrix(ScreenWorldMatrix, ScreenViewMatrix, ViewWorldMatrix);
        sceVu0MulMatrix(ScreenLocalMatrix, ScreenWorldMatrix, WorldLocalMatrix);
        ...
        set_square_packet(&packet, c0[0], v0[5], v0[3], v0[7], v0[1]);
        set_square_packet(&packet, c0[1], v0[0], v0[6], v0[1], v0[7]);
        set_square_packet(&packet, c0[2], v0[2], v0[4], v0[0], v0[6]);
        set_square_packet(&packet, c0[3], v0[3], v0[2], v0[1], v0[0]);
        set_square_packet(&packet, c0[4], v0[3], v0[5], v0[2], v0[4]);
        set_square_packet(&packet, c0[5], v0[4], v0[5], v0[6], v0[7]);
        sceVif1PkCloseGifTag(&packet);         // close GIF Tag
        
        // add packets of frameame
        ...
        // DMA start
        sceDmaSend(dmaVif, (u_long128*)(((u_int)packet.pBase & 0x3ff0) | 0x80000000));
    }
...
void create_normal(sceVu0FVECTOR normal, sceVu0FVECTOR v0, sceVu0FVECTOR v1, sceVu0FVECTOR v2)
{
...
void set_square_packet(sceVif1Packet* packet,  sceVu0IVECTOR c, 
                       sceVu0IVECTOR v0, sceVu0IVECTOR v1,
                       sceVu0IVECTOR v2, sceVu0IVECTOR v3)
{
	float q[4];
    q[0] = 1.0f / (float)v0[3];
    q[1] = 1.0f / (float)v1[3];
    ...
    sceVif1PkAddGsData(packet, SCE_GS_SET_RGBAQ(c[0], c[1], c[2], 0x00, *(u_int*)&q[0]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_XYZ(v0[0], v0[1], v0[2]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_RGBAQ(c[0], c[1], c[2], 0x00, *(u_int*)&q[1]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_XYZ(v1[0], v1[1], v1[2]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_RGBAQ(c[0], c[1], c[2], 0x00, *(u_int*)&q[2]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_XYZ(v2[0], v2[1], v2[2]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_RGBAQ(c[0], c[1], c[2],0x00, *(u_int*)&q[3]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_XYZ(v3[0], v3[1], v3[2]));
}

// make frameame packet routine
void set_line_packet(sceVif1Packet* packet, sceVu0IVECTOR c,
		             sceVu0IVECTOR v0,sceVu0IVECTOR v1 )
{
    float q = 1.0f ;
    sceVif1PkAddGsData(packet, SCE_GS_SET_PRIM(0x01, 0, 0, 0, 0, WALL_AA1, 0, 0, 0));
    sceVif1PkAddGsData(packet, SCE_GS_SET_RGBAQ(c[0], c[1], c[2], c[3], *(u_int*)&q));
    sceVif1PkAddGsData(packet, SCE_GS_SET_XYZ(v0[0], v0[1], v0[2]));
    sceVif1PkAddGsData(packet, SCE_GS_SET_XYZ(v1[0], v1[1], v1[2]));
}
...
void SetRGB(sceVu0IVECTOR* c0, sceVu0FMATRIX LocalLightMatrix, 
	        ...
	        sceVu0FVECTOR* c1)
{
    light_rgb(c0[0], LocalLightMatrix, LightColorMatrix, v1[1], v1[7], v1[3], c1[0]);
    light_rgb(c0[1], LocalLightMatrix, LightColorMatrix, v1[0], v1[6], v1[1], c1[1]);
    light_rgb(c0[2], LocalLightMatrix, LightColorMatrix, v1[2], v1[4], v1[0], c1[2]);
    light_rgb(c0[3], LocalLightMatrix, LightColorMatrix, v1[0], v1[1], v1[2], c1[3]);
    light_rgb(c0[4], LocalLightMatrix, LightColorMatrix, v1[2], v1[3], v1[4], c1[4]);
    light_rgb(c0[5], LocalLightMatrix, LightColorMatrix, v1[4], v1[5], v1[6], c1[5]);
}
